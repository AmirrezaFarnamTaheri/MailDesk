...
def _validate_xlsx_archive(path: Path) -> None:
    """Reject unsafe/corrupt OOXML archives before openpyxl parses XML."""
    try:
        with zipfile.ZipFile(path) as archive:
            ...
            for info in infos:
                ...
                if info.file_size < 0 or info.file_size > MAX_XLSX_MEMBER_BYTES:
                    raise ValueError(
                        f"Spreadsheet archive member {info.filename!r} is too large when expanded."
                    )
                expanded += info.file_size
                if expanded > MAX_XLSX_UNCOMPRESSED_BYTES:
                    raise ValueError(
                        f"Spreadsheet expands beyond the {MAX_XLSX_UNCOMPRESSED_BYTES // 1024 // 1024} MB safety limit."
                    )
...
def _csv_rows(path: Path) -> list[list[str]]:
    raw = path.read_bytes()
    text = None
    # utf-8-sig must be attempted first. A BOM is valid UTF-8, so decoding as
    # plain utf-8 first would leave U+FEFF attached to the first header.
...
def raw_rows(path: Path, sheet: str) -> list[list[str]]:
    # CSV has one logical sheet; callers historically pass the uploaded stem as
    # the sheet name, so intentionally ignore the supplied name for CSV files.
