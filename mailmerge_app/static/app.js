...
const els = {};
const ids = [
  'viewTitle','viewSubtitle','refreshButton','healthChip','queueBadge','versionLabel',
  ...
  'templateSelect','templateName','templateTags','subjectTemplate','bodyTemplate','bodyHtmlEditor','signatureHtmlEditor','placeholderList','snippetList','attachmentList','attachmentFile','newSnippetButton','newTemplateButton','saveTemplateButton','deleteTemplateButton','clearFormattingButton',
...
const state = {
  ...
  importId: '', source: null, headers: [], previewRows: [], allRowNumbers: [], selectedRows: new Set(), rowOverrides: {}, suggestions: {},
  ...
  rendered: null, messageIndex: 0, renderTimer: null, renderRequestId: 0, renderRevision: 0, renderedRevision: -1, batchRequestId: 0, renderDirty: true,
  campaigns: [], verifyingSenderId: '', sourceTab: 'file',
};
...
function clearApproval(){els.reviewedCheck.checked=false;els.confirmText.value='';}
...
async function loadGoogleSheet(){
  ...
  busy(true,'Taking Google Sheet snapshot…','The snapshot is what you review and approve.');
...
async function refreshGoogleSheet(){
  if(!state.importId||state.source?.source_type!=='google_sheet')return; busy(true,'Refreshing Google Sheet…','The current snapshot will be replaced, then messages must be reviewed again.');
  try{
    ...
    toast('Snapshot refreshed','Row edits were cleared. Re-render and review the refreshed snapshot before queueing.','success');
  }
  catch(error){toast('Refresh failed',error.message,'error');} finally{busy(false);}
}
async function applyImport(result){
  clearTimeout(state.renderTimer); state.importId=result.import_id; state.source=result; state.rowOverrides={}; state.selectedRows=new Set(); state.headers=[]; state.previewRows=[]; state.allRowNumbers=[]; invalidateReview({clearRendered:true});
...
async function renderCampaign(showToast=true){
  ...
  try{
    ...
    state.rendered=result;state.renderedRevision=revision;state.renderDirty=false;state.messageIndex=Math.min(state.messageIndex,Math.max(0,result.messages.length-1));clearApproval();renderValidation();renderReview();updateFinalSummary();
...
function renderReview(){
  ...
  const names=(m.attachments||[]).map(id=>state.attachments.find(a=>a.id===id)?.name||id);els.reviewMeta.innerHTML=`<span>${esc(m.display_name||'')}</span><span>${names.length?`${names.length} attachment${names.length===1?'':'s'}: ${esc(names.join(', '))}`:'No attachments'}</span>`;
}
...
async function commitReviewEdit(){
  const m=state.rendered?.messages?.[state.messageIndex];if(!m)return;clearApproval();
  ...
  const preserved=(m.errors||[]).filter(e=>!/^Recipient is blank\.$/.test(e)&&!/^Invalid (recipient|Cc|Bcc|address):/.test(e));m.errors=[...preserved];
  const invalid=[...splitAddressClient(m.to),...splitAddressClient(m.cc),...splitAddressClient(m.bcc)].filter(a=>!validEmailClient(a));if(!splitAddressClient(m.to).length)m.errors.push('Recipient is blank.');if(invalid.length)m.errors.push(`Invalid address: ${invalid.join(', ')}`);
...
function splitAddressClient(v){return String(v||'').split(/[;,\n]+/).map(x=>x.trim()).filter(Boolean);}function validEmailClient(v){return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v);}
...
async function queueCampaign(){
  ...
  const payload={name:els.campaignName.value.trim(),source_name:state.source?.filename||'',mode,account:els.gmailAccount.value||'',browser_sender_id:els.browserSender.value||'',batch_id:r.batch_id,messages:stripSources(r.messages),skip_duplicates:els.skipDuplicates.checked,throttle_ms:Number(els.throttleMs.value||0),scheduled_at:scheduled,reviewed:els.reviewedCheck.checked,confirm_text:els.confirmText.value};busy(true,scheduled?'Scheduling campaign…':'Queueing campaign…',`${r.total} messages`);try{const campaign=await api('/api/campaigns',jsonOptions('POST',payload));toast(scheduled?'Campaign scheduled':'Campaign queued',`${campaign.name} · ${campaign.status}`,'success',6500);clearApproval();setView('queue');await refreshQueue();}catch(error){toast('Campaign was not queued',error.message,'error',9000);}finally{busy(false);updateFinalSummary();}
}
...
function editBrowserSender(s){els.browserSenderId.value=s.id;els.browserSenderLabel.value=s.label;els.gmailSlot.value=s.gmail_slot;els.browserExpectedEmail.value=s.expected_email;const p=state.browserProfiles.find(x=>x.browser_id===s.browser_id&&x.profile_dir===s.profile_dir);if(p)els.browserProfile.value=p.profile_id;updateProfileEmailHints();window.scrollTo({top:0,behavior:'smooth'});}
function resetBrowserSenderForm(){els.browserSenderId.value='';els.browserSenderLabel.value='';els.gmailSlot.value=0;els.browserExpectedEmail.value='';if(state.browserProfiles[0])els.browserProfile.value=state.browserProfiles[0].profile_id;updateProfileEmailHints();}
async function saveBrowserSender(){const profile=state.browserProfiles.find(p=>p.profile_id===els.browserProfile.value);if(!profile){toast('No browser profile','Rescan installed browsers first.','error');return;}const id=els.browserSenderId.value||`route-${Date.now()}`;const payload={id,label:els.browserSenderLabel.value.trim()||`${els.browserExpectedEmail.value.trim()} browser`,browser_id:profile.browser_id,profile_dir:profile.profile_dir,gmail_slot:Number(els.gmailSlot.value||0),expected_email:els.browserExpectedEmail.value.trim()};try{await api(`/api/browser-senders/${encodeURIComponent(id)}`,jsonOptions('PUT',payload));resetBrowserSenderForm();await refreshAccountsAndBrowsers();toast('Browser route saved',`Gmail /u/${payload.gmail_slot}/ → ${payload.expected_email}`,'success');}catch(error){toast('Could not save route',error.message,'error');}}
async function deleteBrowserSender(id){if(!confirm('Delete this browser sender route?'))return;try{await api(`/api/browser-senders/${encodeURIComponent(id)}`,{method:'DELETE'});await refreshAccountsAndBrowsers();}catch(error){toast('Could not delete route',error.message,'error');}}
async function startVerifyBrowserSender(id){state.verifyingSenderId=id;try{const result=await api(`/api/browser-senders/${encodeURIComponent(id)}/verify/open`,{method:'POST'});els.verifyBrowserText.innerHTML=`Check the browser window that just opened. It must show <strong>${esc(result.expected_email)}</strong> at Gmail <code>/u/${result.gmail_slot}/</code>.`;els.verifyBrowserDialog.showModal();}catch(error){state.verifyingSenderId='';toast('Could not open Gmail route',error.message,'error');}}
async function confirmBrowserVerified(){if(!state.verifyingSenderId)return;try{await api(`/api/browser-senders/${encodeURIComponent(state.verifyingSenderId)}/verify/confirm`,{method:'POST'});await refreshAccountsAndBrowsers();toast('Browser route verified','This verification is valid for 30 minutes.','success');}catch(error){toast('Could not record verification',error.message,'error');}finally{state.verifyingSenderId='';}}
...
async function duplicateCampaign(id){
  try{
    ...
    state.renderRevision+=1;state.renderedRevision=state.renderRevision;state.renderDirty=false;state.source=state.rendered.source;state.messageIndex=0;els.campaignName.value=`Copy of ${c.name}`;
    ...
    clearApproval();renderValidation();renderReview();updateModeUI();setView('campaign');setStep(5);
    toast('Campaign duplicated','Review the copied messages and approve a new batch before queueing.','success',7000);
  }catch(error){toast('Could not duplicate campaign',error.message,'error');}
}

async function toggleCampaignDetails(card,id){let existing=q('.queue-items',card);if(existing){existing.remove();return;}try{const c=await api(`/api/campaigns/${id}`);const wrap=document.createElement('div');wrap.className='table-scroll queue-items';const table=document.createElement('table');table.className='data-table';table.innerHTML='<thead><tr><th>#</th><th>Row</th><th>Recipient</th><th>Subject</th><th>Status</th><th>Attempts</th><th>Error</th></tr></thead>';const body=document.createElement('tbody');c.items.forEach(i=>{const tr=document.createElement('tr');[i.ordinal,i.row_number,i.recipient,i.subject,i.status,i.attempts,i.error||''].forEach(v=>{const td=document.createElement('td');td.textContent=v;td.title=v;tr.append(td);});body.append(tr);});table.append(body);wrap.append(table);card.append(wrap);}catch(error){toast('Could not load campaign details',error.message,'error');}}
...
function bindEvents(){
  ...
  qsa('input[name="mode"]').forEach(x=>x.addEventListener('change',updateModeUI));els.browserSender.addEventListener('change',()=>{clearApproval();updateBrowserSenderSummary();updateFinalSummary();});els.gmailAccount.addEventListener('change',()=>{clearApproval();updateFinalSummary();});els.throttleMs.addEventListener('input',()=>{els.throttleMs.dataset.touched='1';});els.scheduleAt.addEventListener('change',()=>{clearApproval();updateFinalSummary();});els.reviewedCheck.addEventListener('change',updateFinalSummary);els.confirmText.addEventListener('input',updateFinalSummary);els.refreshBeforeReview.addEventListener('change',updateFinalSummary);els.queueCampaignButton.addEventListener('click',queueCampaign);els.exportReviewButton.addEventListener('click',exportReviewCsv);
